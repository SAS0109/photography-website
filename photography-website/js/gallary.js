...
// seemore button in website 
  seeMore1.click(function(){
    if(PotFilterOff){
      cd += limit;// next set of images to be displayed.
      if (cd - limit == limit) { // exceeds the limit.
        var x = cd - limit; 
      }else {var x = cd - limit +1; numOfImg += 1;}

      for (var i2 = x; i2 < mainData.length; i2++) {// checking if its full to see if u we can display more images or not.
    ...
    }else { //if potfilteroff is not true then 
      cd+=limit;
        var subMainData = mainData.filter( //we are filtering the main data for the particular data and storing it in submaindata. 
        function(item) {
            return item.type == PText;// select the photos based on filter.
        });
    ...
    potFilter.click(function(e){ 
      ...
      galleryOne.fadeTo(100, .1);//opaque is required to blur out the other images 
